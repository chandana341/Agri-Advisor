...
# Define your static input data for Page 1
user_input = {
    "location": "",
    "crop": "",
    "field_area": 0,
...
# Define a list of static fertilizer recommendations and crop care tips
fertilizer_recommendations = {
    "Wheat": "Use nitrogen-based fertilizers during the growth phase.",
    "Rice": "Apply phosphorus-rich fertilizers to support root development.",
    # Add more crop-specific recommendations
}

# Define multiple output scenarios with random pros and cons for Page 1
output_scenarios = [
    {
        "pros": "Good weather conditions and high market demand.",
        "cons": "Risk of unexpected weather changes and market fluctuations.",
        "suggested_crops": ["Wheat", "Barley"],
    },
    {
        "pros": "Favorable soil conditions and stable market prices.",
        "cons": "Moderate weather conditions and competition in the market.",
        "suggested_crops": ["Maize", "Rice"],
    },
    {
        "pros": "Excellent soil quality and government subsidies.",
        "cons": "High competition in the market and potential pests.",
        "suggested_crops": ["Soybeans", "Sunflowers"],
    },
    {
        "pros": "Low production costs and strong market demand.",
        "cons": "Extreme weather conditions and market fluctuations.",
        "suggested_crops": ["Corn", "Oats"],
    },
    {
        "pros": "Moderate weather conditions and good market demand.",
        "cons": "Average soil quality and potential pests.",
        "suggested_crops": ["Barley", "Rye"],
    },
]
...
disease_info = {
    "Leaf Spot": {
        ...
        "tips": ["Ensure good drainage.", "Properly space the plants."]
    },
    "Powdery Mildew": {
        "pesticide": "Sulfur",
        "tips": ["Prune infected leaves.", "Provide adequate air circulation."]
    },
    "Rust": {
        "pesticide": "Copper-based fungicides",
        "tips": ["Remove and destroy infected leaves.", "Avoid overhead watering."]
    },
    # Add more disease information
}

# Function to analyze crop suitability based on user input and a scenario for Page 1
def analyze_crop_suitability(user_input, scenario):
    # You can use user input and the selected scenario to generate output
    suitability = "Suitable" if random.choice([True, False]) else "Not Suitable"

    return {
        "crop_suitability": suitability,
        "pros": scenario.get("pros", "No pros information available"),
        "cons": scenario.get("cons", "No cons information available"),
        "suggested_crops": ", ".join(scenario.get("suggested_crops", [])),
    }
...
def crop():
    if request.method == 'POST':
        # Handle POST request as you did before
        user_input["location"] = request.form.get('location')
        user_input["crop"] = request.form.get('crop')
        user_input["field_area"] = int(request.form.get('field_area'))
        user_input["previous_crop"] = request.form.get('previous_crop')

        # Select a random output scenario for Page 1
        scenario = random.choice(output_scenarios)

        # Analyze crop suitability based on user input and the selected scenario
        crop_analysis = analyze_crop_suitability(user_input, scenario)

        return render_template('crop-result.html', location=user_input["location"],
                               crop=user_input["crop"], field_area=user_input["field_area"],
                               previous_crop=user_input["previous_crop"], prediction=crop_analysis["crop_suitability"],
                               pros=crop_analysis["pros"], cons=crop_analysis["cons"],
                               suggested_crops=crop_analysis["suggested_crops"])
    else:
        # Handle GET request to access the page
        return render_template('crop.html', location=user_input["location"])

# Your fertilizer_dic data
fertilizer_dic = {
    'A': [
        "Add manure, use coffee grounds, plant nitrogen-fixing plants, grow 'green manure' crops, use mulch. Add sawdust or woodchips, plant heavy nitrogen-feeding plants, soak soil with water, add sugar, use composted manure, plant nitrogen-fixing plants, use NPK fertilizers with high nitrogen, consider doing nothing. Avoid adding manure, use phosphorus-free fertilizer, water soil liberally, plant nitrogen-fixing vegetables, use crop rotations.",
    ],
    'E':[    
        "Use bone meal, apply rock phosphate, use phosphorus fertilizers, add organic compost, use manure, introduce clay particles, ensure proper soil pH, adjust pH with lime or potassium carbonate. Loosen soil and water to dissolve water-soluble potassium, sift through soil to remove rocks, stop applying potassium-rich fertilizer, mix crushed eggshells, crushed seashells, wood ash, or soft rock phosphate, use NPK fertilizers with low potassium, grow a cover crop of legumes. Mix in muriate of potash or sulfate of potash, try kelp meal or seaweed, try Sul-Po-Mag, bury banana peels, use potash fertilizers."
    ],
    'B': [
        "Use nitrogen-rich fertilizers during the growth phase, consider legume cover crops. Apply organic nitrogen sources such as compost, plant nitrogen-fixing crops like clover, rotate with leguminous plants. Use phosphorus-rich fertilizers for root development, maintain optimal soil pH.",
    ],
    'F':[
        "Add bone meal or rock phosphate, use phosphorus-containing organic fertilizers, maintain proper soil pH. Use potassium-rich fertilizers, maintain soil moisture, consider adding wood ash. Apply potassium-containing fertilizers, use organic matter like compost, consider adding banana peels."
    ],
    'C': [
        "Use balanced fertilizers with a higher nitrogen ratio. Apply compost or well-rotted manure, use nitrogen-rich organic fertilizers. Use phosphorus-rich fertilizers for fruit development.",
    ],
    'G':[
        "Add bone meal or superphosphate, use phosphorus-containing organic fertilizers. Use potassium-rich fertilizers, maintain consistent soil moisture. Apply potash-containing fertilizers, add banana peels to the soil."
    ],
    'D': [
        "Use balanced fertilizers with a higher nitrogen ratio. Apply well-rotted manure or compost, use nitrogen-rich organic fertilizers. Use phosphorus-rich fertilizers for root development.",
    ],
    'H':[
        "Add bone meal or rock phosphate, use phosphorus-containing organic fertilizers. Use potassium-rich fertilizers, maintain soil moisture. Apply potash-containing fertilizers, add wood ash to the soil."
    ],
    # Add more crops and their recommendations
}
...
def fertilizer():
    if request.method == 'POST':
        ...
        # Fetch all points for the selected condition and crop
        recommendations = fertilizer_dic.get(condition, ["No recommendations available"])
        recommendations = [rec.replace("--", " ") for rec in recommendations] if recommendations else ["No recommendations available"]

        return render_template('fertilizer-result.html', crop=crop, recommendations=recommendations, condition=condition)

    # If it's a GET request, simply render the fertilizer.html template
...
def disease():
    if request.method == 'POST':
        # Perform image processing and disease prediction here
        # For simplicity, we'll randomly choose a disease from disease_dic
        predicted_disease = random.choice(list(disease_dic.keys()))

        # Get the disease information from disease_dic
        disease_info = disease_dic.get(predicted_disease, "No information available for this disease")

        return render_template('disease-result.html', disease_info=disease_info)

    # If it's a GET request, simply render the disease.html template
